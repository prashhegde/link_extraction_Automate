...
class ISmartBuyGlasses:
    def __init__(self, website, brands, excel, excel_dump, driver, logging, match_product_name, get_page, total_links):
        get_page(
            "http://" + website + "/contact-lenses/m/Alcon#!&s=popularity&tb=0&b=Acuvue-Air+Optix-Biofinity-Biomedics-BioTrue-Clariti-Dailies-Frequency-Freshlook-Proclear-PureVision-Soflens&m=Alcon-Bausch+%26+Lomb-Coopervision-Johnson+%26+Johnson&p=1")
            ...
            try:
                ...
                try:
                    next_button = self.driver.find_element_by_xpath(
                        '//*/li[contains(@class,"pgNext")][3]')
                    # print(load_more_button.get_attribute("innerHTML"))
                    if str(next_button.get_attribute("class")).__contains__("pgEmpty"):
                        break
    ...
    def get_data(self, div, excel, excel_dump):
        ...
        try:
            ...
            pass
        try:
            content = soup.select_one(".cl-pro-name")
            if content is not None:
