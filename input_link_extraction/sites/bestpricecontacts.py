...
class IBestPriceContacts:
    def __init__(self, website, brands, excel, driver, logging, match_product_name, get_page, total_links):
        ...
        for brand in self.brands:
            get_page("http://" + website)
            search_input = self.driver.find_element_by_xpath('/html/body/div[3]/main/div[3]/div/div/div[2]/div/div[2]/div[3]/form/input[4]')
            search_input.clear()
            ...
            while True:
                ...
                for div in elements:
                    try:
                        last_thread = threading.Thread(target=self.get_data,args=(div.get_attribute("innerHTML"), excel))
                        last_thread.start()
                    except:
                        self.total_exp += 1
    ...
    def get_data(self, div, excel, excel_dump):
        ...
        try:
            ...
            if content is not None:
                attributes.append("https://www.bestpricecontacts.com" + str(content.get("href")))
            else:
