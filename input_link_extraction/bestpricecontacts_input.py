...
from input_link_extraction.sites.bestpricecontacts import IBestPriceContacts
import threading
...
class LinkExtractor:
    ...
    def extract_data(self):
        ...
        website="www.bestpricecontacts.com"
        ...
        IBestPriceContacts(website, self.brands, excel, self.driver, self.logging,self.match_product_name,self.get_page, self.total_links)

            #excel_dump.add_headers(excel_dump.wb.active,["Product Link", "Product Name", "No. of Reviews"])

        threading.Thread(target=self.save_excels, args=(excel)).start()
