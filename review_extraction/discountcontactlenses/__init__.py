...
from review_extraction.discountcontactlenses import *
from pathlib import Path

TAG = "www.discountcontactlenses.com"
PATH_INPUT = "C:/Users/Prash/Desktop/Work/scrapj-new/scrapj/data/input/"


class DiscountCLenses():
    driver = None
    total_reviews = 0
    total_exp = 0
    start_time = ''
    ...
    def extract_data(self, excel):
        wb = load_workbook(filename=PATH_INPUT + 'www.discountcontactlenses.com.xlsx', read_only=True)
    ...
    def grab_reviews(self, excel, product_name, product_url):
        ...
        prev_div = ''
        try:
            product_name = self.driver.find_element_by_xpath('//*[@class="dcl-lens-detail__lens-info-image"]/div/h1')
...
wg = Excel("www.discountcontactlenses.com") #correct
wb = getattr(wg, "wb")
lens = DiscountCLenses(driver, None)
lens.extract_data(wg)
while True:
    try:
        wg.save_xls(wb)
        Path("../status/" + "www.discountcontactlenses.com" + ".txt").touch()
        break
    except:
        pass