...
from review_extraction.framesdirect import *
from pathlib import Path
import os

TAG = "www.framesdirect.com"
...
class FramesDirect():
    ...
    def extract_data(self, excel):
        wb = load_workbook(filename=PATH_INPUT + 'www.framesdirect.com.xlsx', read_only=True)
    ...
    def grab_reviews(self, excel, product_name, product_url):
        ...
        print(len(self.driver.find_elements_by_xpath('//*[@class="review-block"]')))
        log = [product_name, product_url]
        curr_reviews = self.total_reviews
        last_thread = None
        try:
            product_name = self.driver.find_element_by_xpath('//*[@id="itemPlaceholderContainer"]/div/div/strong/text()')
            product_name = str(product_name.get_attribute("innerHTML")).strip()
        except:
            print("Error getting product name")
            pass
        for div in self.driver.find_elements_by_xpath('//*[@id="itemPlaceholderContainer"]/div'):
            try:
    ...
    def process_soup(self, div):
        ...
        try:
            content = soup.select_one(".review-header strong")
            ...
            pass
        try:
            content = soup.select_one(".review-body")
            if content is not None:
                print(content.getText())
                attributes.append(content.getText())
            else:
            ...
            pass
        try:
            content = soup.select(".icon-star-solid")
            if content is not None:
        ...
        attributes.append("NA3")
        try:
            content = soup.select_one(".review-name")
            if content is not None:
        ...
        try:
            content = soup.select_one(".review-date")
            if content is not None:
...
wg = Excel("www.framesdirect.com") #correct
wb = getattr(wg, "wb")
lens = FramesDirect(driver, None)
lens.extract_data(wg)
while True:
    try:
        wg.save_xls(wb)
        Path("../status/" + "www.framesdirect.com" + ".txt").touch()
        break
    except:
        pass